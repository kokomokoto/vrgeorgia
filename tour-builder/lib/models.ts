...
/**
 * tour-builder-ის მონაცემები MongoDB-ში (იგივე ბაზა, რასაც მთავარი საიტი იყენებს).
 * ლოგიკური გასაღები — string `id` (uuid), რომ ყველა არსებული ბმული (tour_id,
 * scene_id, target_scene_id) უცვლელად დარჩეს. _id/__v გარეთ არ გადის (იხ. mappers).
 */
...
// hot-reload-ზე მოდელი ხელახლა არ კომპილდეს
...
// ─── mappers: Mongo doc → მკაცრი ტიპი (ფანჯრის _id/__v გარეშე) ───

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function toTour(d: any): Tour {
  return {
    id: d.id,
    title: d.title,
    slug: d.slug,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function toSceneRaw(d: any): Scene {
  return {
    id: d.id,
    tour_id: d.tour_id,
    name: d.name,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function toHotspot(d: any): Hotspot {
  return {
    id: d.id,
    scene_id: d.scene_id,
