      ...
      {open && (
        <div className="border-t border-slate-200 bg-white md:hidden dark:border-zinc-800 dark:bg-black">
          <div className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-3">
            <Link href="/services" onClick={() => setOpen(false)} className="flex flex-row items-center gap-2 py-2 text-sm text-slate-800 dark:text-zinc-100">
              <svg className="h-4 w-4 shrink-0 text-amber-600" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
            ...
            <Link href="/upload" onClick={() => setOpen(false)} className={`${uploadLinkClass} py-2`}>
              {uploadText}
            </Link>
            <Link href="/favorites" onClick={() => setOpen(false)} className="flex flex-row items-center gap-2 py-2 text-sm text-slate-800 dark:text-zinc-100">
              <svg className="h-4 w-4 shrink-0 text-red-500" width="16" height="16" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                <path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
              {favoritesText}
            </Link>
            <Link href="/compare" onClick={() => setOpen(false)} className="flex flex-row items-center gap-2 py-2 text-sm text-slate-800 dark:text-zinc-100">
              <svg className="h-4 w-4 shrink-0 text-blue-500" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
            ...
            {!user ? (
              <Link href="/login" onClick={() => setOpen(false)} className="py-2 text-sm font-medium text-blue-600">
                {loginText}
              </Link>
            ) : (
              <>
                <Link href="/messages" onClick={() => setOpen(false)} className="flex flex-row items-center gap-2 py-2 text-sm text-slate-800 dark:text-zinc-100">
                  <svg className="h-4 w-4 shrink-0 text-green-500" width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                  </svg>
                  {messagesText}
                </Link>
                <Link href="/profile" onClick={() => setOpen(false)} className="py-2 text-sm font-medium text-blue-600">
                  {profileText}
                </Link>
                {isAdmin && (
                  <Link href="/admin" onClick={() => setOpen(false)} className="flex flex-row items-center gap-2 py-2 text-sm font-semibold text-rose-600">
