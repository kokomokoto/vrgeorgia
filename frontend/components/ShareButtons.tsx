...
interface ShareButtonsProps {
  ...
  title: string;
  description?: string;
  /** inline — ჰორიზონტალური; sidebar — მარჯვე პანელისთვის */
  variant?: 'inline' | 'sidebar';
}

const SOCIAL_BTN_CLASS: Record<SiteSocialNetwork, string> = {
  facebook: 'bg-blue-600 text-white hover:bg-blue-700',
  instagram: 'bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 text-white hover:from-purple-700 hover:via-pink-600 hover:to-orange-500',
  twitter: 'bg-black text-white hover:bg-gray-800',
  whatsapp: 'bg-green-500 text-white hover:bg-green-600',
  telegram: 'bg-sky-500 text-white hover:bg-sky-600',
...
const SOCIAL_LABEL: Record<SiteSocialNetwork, string> = {
...
function withListingContext(
  ...
  if (!listingUrl) return href;
  try {
    const u = new URL(href);
    const text = [listingTitle, listingUrl].filter(Boolean).join('\n');
    if (network === 'whatsapp' && text) {
...
function SocialIcon({ network }: { network: SiteSocialNetwork }) {
  if (network === 'facebook') {
    return (
...
export function ShareButtons({ url, title, variant = 'inline' }: ShareButtonsProps) {
  ...
  const socialButtons = socials.map(({ network, href }) => (
    <a
      key={network}
      href={withListingContext(network, href, url, title)}
      target="_blank"
      rel="noopener noreferrer"
      className={`${iconBtn} ${SOCIAL_BTN_CLASS[network]}`}
  ...
  const copyIconButton = (
    <button
      ...
      onClick={handleCopy}
      className={`${iconBtn} ${
        copied ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'
      }`}
  ...
  if (variant === 'sidebar') {
    return (
      <div className="space-y-3">
        <div className="text-sm font-semibold text-slate-900">{t('writeUs')}</div>
        {socialButtons.length > 0 ? (
          <div className="flex flex-wrap justify-center gap-2 sm:justify-start">{socialButtons}</div>
        ) : null}
        <button
          type="button"
          onClick={handleCopy}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="mr-2 text-sm text-slate-600">{t('writeUs')}:</span>
      {socialButtons}
      {copyIconButton}
    </div>
  );
